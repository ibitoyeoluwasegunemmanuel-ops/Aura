...
// ── REAL CLAUDE API ───────────────────────────────────────────────────────────
const CLAUDE_KEY = process.env.REACT_APP_ANTHROPIC_API_KEY || "";
...
function AdminGate({ onUnlock }) {
  ...
  const check = () => {
    if (pw === sto.get("admin_pw", "aura2024")) {
      onUnlock();
    } else {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", padding: 32 }}>
      ...
      <div style={{ width: "100%", maxWidth: 300, animation: shake ? "shake 0.5s ease" : "none" }}>
        ...
        <div style={{ fontSize: 9, color: "rgba(255,255,255,0.12)", textAlign: "center", marginTop: 10 }}>Default: aura2024</div>
      </div>
    </div>
  );
}
